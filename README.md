...
ReviveRelay client version `0.4.3` connects the centrally pooled public-chat candidate data to the Reviver tab. Fresh revive candidates observed by any connected collector appear in a separate **Shared public chat requests** feed for connected ReviveRelay users, while direct protected marketplace requests remain a distinct verification-gated queue. The shared feed exposes only the approved public-request projection (public channel, player ID/name, request text, observation times/count) and keeps dedupe keys, classifier metadata, and source-message identifiers server-side. Version `0.4.2` introduced the movable, persistent four-tab hybrid UI, and the hardened automatic/manual release channels from `0.4.1` remain unchanged. Every generated `@require` URL is pinned to the exact 40-hex GitHub release commit, and client telemetry carries that same build commit. The release command rejects moving/stale dependency refs and verifies each pinned GitHub support module byte-for-byte against the committed local source before a manifest can be published. Automatic installations use Tampermonkey's native `@updateURL`/`@downloadURL` behavior; manual installations disable native updates and receive a once-daily JSON manifest check plus an install link. ReviveRelay never downloads and `eval()`s executable updates and never rewrites its own userscript.
