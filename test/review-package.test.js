...
const requiredDocs=[
  ...
  'docs/review/REVIVERELAY-0.6.0-STAFF-SUMMARY.md','docs/review/ENDPOINT-INVENTORY.md',
...
test('Torn review package contains all 19 approved evidence classes',()=>{
  ...
  assert.ok(exists('docs/review/REVIVERELAY-0.6.0-AUTOMATED-VERIFICATION.md'),'18 automated report');
  ...
  const evidence=[
    ...
    'docs/review/BUILD-MANIFEST.json','docs/review/REVIVERELAY-0.6.0-AUTOMATED-VERIFICATION.md','docs/review/REVIVERELAY-0.6.0-STAFF-SUMMARY.md'
  ];
...
test('screenshot checklist requires human evidence for all five 0.6.0 surfaces and does not claim completion',()=>{
...
test('review docs accurately describe immutable review channel without claiming stable 0.6.0 promotion',()=>{
  const summary=read('docs/review/REVIVERELAY-0.6.0-STAFF-SUMMARY.md');
  assert.match(summary,/private review channel/i);
  assert.match(summary,/public production[^\n]*0\.4\.4/i);
  assert.match(summary,/review[^\n]*(?:only|channel)/i);
  assert.doesNotMatch(summary,/0\.6\.0[^\n]*(?:is|now) (?:live|public stable|production)/i);
});
