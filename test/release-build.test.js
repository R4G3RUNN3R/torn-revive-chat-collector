...
test('build produces exact immutable review 0.6.3 artifact with review metadata',()=>{
  const file=`dist/review/ReviveRelay-${pkg.version}.user.js`;
  assert.ok(fs.existsSync(file),file);
  const text=fs.readFileSync(file,'utf8');
  assert.equal(text.match(/@version\s+(\S+)/)?.[1],'0.6.3');
  ...
  assert.match(text,/@updateURL\s+https:\/\/reviverelay\.voidsmithindustries\.com\/releases\/review\/0\.6\.3\/ReviveRelay-0\.6\.3\.meta\.js/);
  assert.match(text,/@downloadURL\s+https:\/\/reviverelay\.voidsmithindustries\.com\/releases\/review\/0\.6\.3\/ReviveRelay-0\.6\.3\.user\.js/);
