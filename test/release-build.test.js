...
test('build produces ReviveRelay 0.6.9 review artifact with permanent review distribution update URLs',()=>{
  assert.equal(pkg.version,'0.6.9');
  const file='dist/review/ReviveRelay-0.6.9.user.js';
  assert.ok(fs.existsSync(file),file);
  const text=fs.readFileSync(file,'utf8');
  assert.equal(text.match(/@version\s+(\S+)/)?.[1],'0.6.9');
...
test('current-version pinned manifest makes normal builds reproducible',()=>{
  ...
  assert.deepEqual(build.resolveBuildIdentity({
    ...
    headCommit:head,
    nowTimestamp:fresh,
    manifest:{version:'0.6.9',artifactSourceCommit:pinned,buildTimestamp:frozen}
  ...
  assert.throws(()=>build.resolveBuildIdentity({
    version:'0.6.9',headCommit:head,nowTimestamp:fresh,
    manifest:{version:'0.6.9',artifactSourceCommit:'bad',buildTimestamp:frozen}
